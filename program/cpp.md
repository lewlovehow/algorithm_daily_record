...
| 赋值语义     | 复制语义                    | 移动语义             |
7 map 与unordermap区别 简述底层实现 各自在什么场合适合使用？

### ✅ 标准面试答题模板

#### **问题**

> `std::map` 与 `std::unordered_map` 有什么区别？简述其底层实现机制，各适用于哪些使用场景？
...
| ------------ | ----------------- | ----------------------- |
| 键值存储是否有序     | ✅ 有序（按 key 排序）    | ❌ 无序（key 无顺序）           |
| 底层实现结构       | 平衡二叉搜索树（通常为红黑树）   | 哈希表（Hash Table）         |
| 查找、插入、删除复杂度  | O(logN)           | O(1) 平均，O(N) 最坏（哈希冲突严重） |
| 支持范围查询（区间查找） | ✅ 支持              | ❌ 不支持                   |
| key 的要求      | 必须可排序（定义 `<` 运算符） | 必须可哈希（定义 `hash` 函数）     |
| 内存使用         | 较少                | 较多（需维护哈希桶）              |
...
* 基于 **红黑树（Red-Black Tree）** 实现，自动维护键值对有序。
* 插入/删除/查找操作为 **O(logN)**。
* 支持 **中序遍历** 与范围查找（例如查找一个范围内所有 key 的元素）。

##### 🔸 `std::unordered_map`

* 基于 **哈希表（Hash Table）** 实现，数据按哈希分桶（bucket）组织。
* 插入/删除/查找操作为 **O(1)** 平均时间复杂度，最坏为 O(N)。
* 键需要提供哈希函数和相等比较函数。
...
| 场景                   | 推荐容器                 | 原因说明               |
| -------------------- | -------------------- | ------------------ |
| 需要保持 key 的有序性或支持范围查找 | `std::map`           | 有序结构，适合中序遍历、区间查找等  |
| 追求查找/插入效率，无序即可       | `std::unordered_map` | 哈希结构，查找插入快，常用于频繁查找 |
| key 类型为复杂自定义类型       | `std::map`           | 只需实现 `<` 运算符，使用简单  |
| key 类型为整数、字符串等原生类型   | `std::unordered_map` | 标准库已提供哈希函数，使用方便    |
...
> 输出中 `map` 会自动排序，而 `unordered_map` 则不保证顺序。
...
* 有序 → `map`（红黑树，O(logN)）
* 无序快 → `unordered_map`（哈希表，O(1)）
* 查范围、遍历有序 → 用 `map`
* 查存在性、频繁查找 → 用 `unordered_map`
